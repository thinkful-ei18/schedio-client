...
export default class WeatherWidget extends React.Component {
  ...
  componentDidMount() {
	...
	getWeather = () => {
	  ...
	  const calculatedDiff = moment(Number(this.props.event.starttime)).diff(Date.now() , 'days');
	    ...
	    method: 'GET'
	      ...
	      if (daysAhead < 7 && daysAhead > 0) {
	        ...
	        this.setState({
	          ...
	          temperature:
							response.data.forecast.forecastday[daysAhead <= 0 ? 0 : daysAhead].day.avgtemp_f,
	          conditionImg:
							response.data.forecast.forecastday[daysAhead <= 0 ? 0 : daysAhead].day.condition
							  .icon,
	          conditions:
							response.data.forecast.forecastday[daysAhead <= 0 ? 0 : daysAhead].day.condition
							  .text,
	          high:
							response.data.forecast.forecastday[daysAhead <= 0 ? 0 : daysAhead].day.maxtemp_f,
	          low:
							response.data.forecast.forecastday[daysAhead <= 0 ? 0 : daysAhead].day.mintemp_f,
	          sunrise:
							response.data.forecast.forecastday[daysAhead <= 0 ? 0 : daysAhead].astro.sunrise,
	          sunset:
							response.data.forecast.forecastday[daysAhead <= 0 ? 0 : daysAhead].astro.sunset,
	          date: response.data.forecast.forecastday[daysAhead <= 0 ? 0 : daysAhead].date,
	          loading: false
	        });
